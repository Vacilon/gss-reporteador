...
from src.helpers.processor import dow_policy, sen_policy
import json, os
    ...
    def download_policy():
        if request.args:
            ...
            if isinstance(data, dict):
                ...
                policy_filename = dow_policy(data)
                return send_from_directory(os.getcwd(), policy_filename, attachment_filename="%s.pdf" % (policy_filename))
            else:
    ...
    def send_policy():
        if request.args:
            ...
            if isinstance(data, dict):
                ...
                policy_to = policy_from + timedelta(weeks=52)
                data['policy_to'] = policy_to.strftime("%d-%m-%Y")
                policy_filename = sen_policy(data)
                return policy_filename, 200
            else:
                return jsonify({"msg": "error while trying to create the policy"}), 500
        else:
            return jsonify({"msg": "bad request"}), 400
